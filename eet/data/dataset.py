...
from .types import EyeEpisodeDataType, RGBEpisodeDataType, BatchItemDataType
...
def build_dataset(config: Config, split: str = "train") -> 'MixtureDataset':
  ...
  processor_kwargs = dict(config.data.processor_kwargs[config.data.processor])
  if config.data.processor == 'general':
    processor_kwargs.setdefault('mode', split)
  processor: BaseProcessor = PROCESSOR_CLASS_MAPPING[config.data.processor](**processor_kwargs)
  ...
  for dataset_name in tqdm(
    ...
    datasets = []
    temporal = config.data[dataset_name].temporal
    ...
    for dataset_path in config.data[dataset_name].dataset_paths:
      ...
      # Create Dataset with the loader
      dataset: 'Dataset' = Dataset(
        loader=loader,
        processor=processor,
        sequence_length=config.data.batch_length,
  ...
  ), processor.input_space, processor.label_space
...
def build_dataloader(
  ...
  use_multiprocessing = int(num_workers) > 0
  loader_kwargs = {
    'dataset': dataset,
    ...
    'drop_last': True,
    'pin_memory': True,
    'persistent_workers': bool(persistent_workers and use_multiprocessing),
  }
...
class Dataset:
  ...
  def extract_episode_data(self, idx: int) -> BatchItemDataType:
    """
    Load and process all event bins in a single episode.
    QoL Code for getting the whole sequence as batch item, not used in forming T dimension in dataloder
    This method will be used in the `extract_all_sequences` method to pre-process the episode data
      and split it into timebins

    Args:
      idx: the index of the episode

    Returns:
      BatchItemDataType containing all data bins in the episode
    """
    episode_data: EyeEpisodeDataType | RGBEpisodeDataType = self.loader[idx]

    if isinstance(episode_data, EyeEpisodeDataType):
      total_labels = episode_data.n_labels
      timebin = episode_data.timebin
      width = episode_data.width
      height = episode_data.height
      episode_stream = episode_data.data
      labels = episode_data.labels

      all_timebin_data: List[np.ndarray] = []
      timebin_boundaries = np.arange(total_labels + 1) * timebin
      split_indices = np.searchsorted(episode_stream['t'], timebin_boundaries, side='left')

      for i in range(total_labels):
        start_idx = split_indices[i]
        end_idx = split_indices[i + 1]
        timebin_data = episode_stream[start_idx:end_idx]
        all_timebin_data.append(timebin_data)

      return BatchItemDataType(
        data=all_timebin_data,
        labels=[labels[i] for i in range(total_labels)],
        width=width,
        height=height,
        timebin=timebin,
        batch_length=total_labels,
        modality='event',
        temporal=self.temporal,
        source_path=episode_data.source_path,
      )

    if isinstance(episode_data, RGBEpisodeDataType):
      total_labels = episode_data.n_labels
      frames = episode_data.frames
      labels = episode_data.labels
      return BatchItemDataType(
        data=[frames[i] for i in range(total_labels)],
        labels=[labels[i] for i in range(total_labels)],
        width=episode_data.width,
        height=episode_data.height,
        timebin=episode_data.timebin,
    ...
    raise TypeError(f"Unsupported episode data type: {type(episode_data)}")

  def __getitem__(self, idx: int) -> BatchItemDataType:
    """Will be called in the mixture dataset loader

    Args:
        idx (int): Index of the episode to retrieve

    Returns:
        BatchItemDataType: Batch item containing sequences from the episode
    """
    return self.extract_episode_data(idx)
    # return self.extract_all_sequences(idx)

  def extract_all_steps(self, idx: int) -> List[BatchItemDataType]:
    """
    Extract all single-step samples from an episode (no temporal stacking).
    """
    datapoints: List[BatchItemDataType] = []
    episode_data: BatchItemDataType = self.extract_episode_data(idx)

    for t in range(episode_data.batch_length):
      batch_item = BatchItemDataType(
        data=[episode_data.data[t]],
        labels=[episode_data.labels[t]],
        width=episode_data.width,
        height=episode_data.height,
        timebin=episode_data.timebin,
        batch_length=1,
        modality=episode_data.modality,
        temporal=False,
        source_path=episode_data.source_path,
      )
  ...
  def extract_all_sequences(self, idx: int) -> List[BatchItemDataType]:
    """
    Extract all sequences from an episode. This is the method forming the T dimension in the batch item

    For each episode, we split it into sequences of `sequence_length` timesteps with `stride` offset.
    Each sequence is a BatchItemDataType containing data and labels for consecutive timebins.

    NOTE: This method will be called in the mixture dataset

    Args:
      idx: the index of the episode

    Returns:
      List of BatchItemDataType, one per sequence in the episode
    """

    if not self.temporal:
      return self.extract_all_steps(idx)

    datapoints: List[BatchItemDataType] = []
    # The batch item data type here is a very big batch, containing the whole episode
    episode_data: BatchItemDataType = self.extract_episode_data(idx)
    all_timebin_data: List[np.ndarray] = episode_data.data
    total_labels = episode_data.batch_length
    timebin = episode_data.timebin
    width = episode_data.width
    height = episode_data.height
    labels = episode_data.labels
    ...
    for start_idx in range(0, total_labels - self.sequence_length + 1, self.stride):
      ...
      # Slice pre-processed data for this sequence
      sequence_data = all_timebin_data[start_idx:end_idx]
...
class MixtureDataset(IterableDataset):
  ...
  def cache_next_episode(self, episode_schedule: List[Tuple[int, int]], schedule_idx: int):
    ...
    self._cache_job = self._executor.submit(
      self.datasets[dataset_idx].extract_all_sequences, episode_idx
    )
    ...
    for ds in self.datasets:
      for episode_idx in range(len(ds)):
        ...
        if ds.temporal:
          ...
          total_sequences += episode_length
