...
class CNNBidirectionalGRUAgent(JAXAgent):
  ...
  def __init__(self, obs_space, label_space, config, *args, **kwargs):
    ...
    self.dynamics = nn.BidirectionalGRU(**config.dynamics.gru, name='dyn')
  ...
  def ext_space(self):
    ...
    if self.config.replay_context:
      spaces.update(tree.flatdict(dict(
        dyn=Space(np.float32, self.grunits * 2),
      )))
    return spaces
  ...
  def init_infer(self, batch_size):
    ...
    return (self.dynamics.initial(batch_size), obs)
  ...
  def infer(self, carry, obs, mode="train"):
    ...
    feat = self.encoder(main_obs, main_obs['is_first'], **kw) # (B, T, dim)
    # print(f"[CNNGRU.infer] feat.shape: {feat}")
    dyn_carry, feat = self.dynamics(nn.cast(dyn_carry), feat, main_obs['is_first']) # (B, T, dim)
    feat = feat[:, -1] # (B, dim)
    dist = self.head(feat, bdims=1) # (B, dim)
    result = {k: jax.nn.tanh(v.pred()) for k, v in dist.items()}

    carry = (dyn_carry, main_obs)
    out = {}
    if self.config.replay_context:
  ...
  def _loss(self, carry, data, training=True):
    ...
    dyn_carry, seq_carry = self.dynamics(nn.cast(dyn_carry), feat, reset) # (B, dim), (B, T, dim)
