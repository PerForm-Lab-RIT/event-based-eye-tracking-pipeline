...
class MambaPupilAgent(JAXAgent):
  ...
  def __init__(self, obs_space, label_space, config, *args, **kwargs):
    ...
    # Mamba pupil agent = Spatial (CNN) + Temporal (BiGRU + Mamba) + Head (MLP)
    self.dynamics1 = nn.BidirectionalGRU(**config.dynamics.gru, name='dyn1')
    ...
    self.mamba_state_shape = (config.dynamics.mamba.units * config.dynamics.mamba.expand, config.dynamics.mamba.deter)
  ...
  def ext_space(self):
    ...
    if self.config.replay_context:
      spaces.update(tree.flatdict(dict(
        dyn1=Space(np.float32, self.grunits * 2),
        dyn2=Space(np.float32, self.mamba_state_shape),
      )))
    return spaces

  def init_infer(self, batch_size):
    obs = {k: jnp.zeros((batch_size, self.config.batch_length, *v.shape)).astype(v.dtype)
      for k, v in self.obs_space.items() if k not in self.label_space}
    return ((self.dynamics1.initial(batch_size), self.dynamics2.initial(batch_size)), obs)
  ...
  def infer(self, carry, obs, mode="train"):
    ...
    main_obs = {k: jnp.concat([
      prev_obs_seq[k][:, 1:],
      obs[k][:, None]
    ], axis=1) for k in self.obs_space if k not in self.label_space}
    reset = main_obs['is_first']

    feat = self.encoder(main_obs, reset, **kw) # (B, dim)
    ...
    out = {}
    if self.config.replay_context:
      out.update(tree.flatdict(dict(dyn1=dyn_carry1, dyn2=dyn_carry2)))
  ...
  def _loss(self, carry, data, training=True):
    ...
    dyn_carry1, seq_carry1 = self.dynamics1(nn.cast(dyn_carry1), feat, reset) # (B, dim), (B, T, dim)
    dyn_carry2, seq_carry2, seq_output = self.dynamics2(nn.cast(dyn_carry2), seq_carry1, reset, single=False) # (B, *state_dim), (B, T, *state_dim), (B, T, dim)
    # print(f"[MambaAgent._loss] seq_carry.shape: {seq_carry.shape}", color='green')
    dists = self.head(seq_output, bdims=2) # (B, T, dim)
    losses = {}
  ...
  def train(self, carry, data):
    ...
    if self.config.replay_context:
      updates = tree.flatdict(dict(
        stepid=stepid, dyn1=entries[0][0], dyn2=entries[0][1],))
  ...
  def _apply_replay_context(self, carry, data):
    ...
    entries = [nested.get(k, {}) for k in ('dyn1', 'dyn2')]
    # print(f"[MambaPupilAgent._apply_replay_context] entries: {entries}", color='green')
    ...
    rep_carry = (jax.tree.map(lambda x: x[:, -1], lhs((entries[0], entries[1]))),)
    rep_obs = {k: rhs(data[k]) for k in _obs_space}
    rep_ext_obs = {k: rhs(ext_obs[k]) for k in self.ext_space}
    rep_stepid = rhs(stepid)
    first_chunk = (data['consec'][:, 0] == 0)
    # print(f"[MambaPupilAgent._apply_replay_context] carry: {carry}", color='yellow')
    # print(f"[MambaPupilAgent._apply_replay_context] rep_carry: {rep_carry}", color='green')
