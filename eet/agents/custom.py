...
from modeling.ssm import RSSM, EBSSM
from .utils import *

class CustomAgent(JAXAgent):

  """This agent only implement RSSM model"""
  ...
  def __init__(self, obs_space, label_space, config, *args, **kwargs):
    ...
    self.dynamics = RSSM(**config.dynamics.rssm, name="dyn")
  ...
  def infer(self, carry, obs, mode="train"):
    ...
    carry, dyn_entry, feat = self.dynamics.observe(
      nn.cast(dyn_carry), embed, reset, training=mode=='train', single=True) # (B, dim)
  ...
  def _loss(self, carry, data, training=True):
    ...
    dyn_carry, dyn_entries, los, feat, mets = self.dynamics.loss(dyn_carry, embed, reset, training)
    metrics.update(mets)
