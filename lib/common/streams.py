...
class Stream:
  """Basically an iterator, but with some extra methods for saving and loading."""
...
class Stateless(Stream):
  """Basically an iterator, but with some extra methods for saving and loading.
  Now this iterator is stateless, meaning it doesn't need to save or load any state.
  Furthermore, it allows iterating by continously calling the passed function.
  """
  def __init__(self, nextfn, transform=None, *args, **kwargs):
  ...
  def __next__(self):
    return self.transform(self.nextfn())
...
class Prefetch(Stream):

  """Prefetch data from a source stream.
  """
  ...
  def __init__(self, source, transform=None, amount=1):
    ...
    self.worker = distr.StoppableThread(self._worker, start=False)
    self.started = False
...
class Zip(Stream):
  ...
  def __next__(self):
    ...
    result = tree.map(lambda *el: np.concatenate(el), *parts)
    return result
