...
class Checkpoint:
  ...
  def exists(self, filename=None, verbose=True):
    assert self._filename or filename
    filename = path.Path(filename or self._filename)
    exists = self._filename.exists()
    if verbose:
      if exists:
        printing.print_('Found existing checkpoint.')
      else:
        printing.print_('Did not find any checkpoint.')
    return exists

  def save(self, filename=None, keys=None, verbose=True):
    assert self._filename or filename
    filename = path.Path(filename or self._filename)
    if verbose:
    ...
    if self._parallel:
      ...
      self._promise = self._worker.submit(self._save, filename, keys, verbose)
    else:
      self._save(filename, keys, verbose)

  @timer.section('checkpoint_save')
  def _save(self, filename, keys, verbose=True):
    ...
    if str(filename).startswith('gs://'):
      ...
      tmp.move(filename)
    if verbose:
      printing.print_('Wrote checkpoint.')

  @timer.section('checkpoint_load')
  def load(self, filename=None, keys=None, verbose=True):
    assert self._filename or filename
