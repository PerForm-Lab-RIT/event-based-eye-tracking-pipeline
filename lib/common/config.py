...
def load_config_from_yaml(config: pathlib.Path | path.Path | dict, argv: list = []) -> Config:
  ...
  Args:
      config (str | pathlib.Path | embodied.Path): _description_
      argv (list, optional): _description_. Defaults to [].

  Returns:
      Config: _description_
  """
  assert isinstance(config, (pathlib.Path, path.Path, dict)), "config should be either a path (pathlib.Path or embodied.Path format) or a config dictionary (dict)."
  if isinstance(config, dict):
    configs = config
  else:
    config_path = config
    if isinstance(config_path, path.Path):
      _config_path = config_path.read()
    elif isinstance(config_path, pathlib.Path):
      _config_path = config_path
    configs = yaml.YAML(typ='safe').load(_config_path)
  parsed, other = flags.Flags(configs=['defaults']).parse_known(argv)
  # Preping and parsing all configs and overrides
  config = Config(configs['defaults'])
  for name in parsed.configs:
    config = config.update(configs[name])
  config = flags.Flags(config).parse(other)
  if 'seed' in config and not config.seed:
    config.update({'seed': random.randint(0, 2**20)})
  ...
  if "logroot" in config and "expname" in config and "logdir" in config and config.logroot and config.expname and config.logdir == "":
    logdir = path.Path(config.logroot) / config.expname
    logdir.mkdir()
  ...
  # Or simply take the logdir if the logdir is available
    ...
    # Save the config inside logdir
    config.save(logdir / 'config.yaml')
  elif "logroot" not in config or not config.logroot:
    logroot = pathlib.Path(__file__).parent / "logs"
    logroot.mkdir(exist_ok=True)
    config = config.update({"logroot": str(logroot)})
    expname = time.strftime("%Y%m%d_%H%M%S")
    config = config.update({"expname": expname})
    logdir = logroot / expname
    logdir.mkdir(exist_ok=True)
    config = config.update({"logdir": str(logdir)})
    config.save(logdir / 'config.yaml')
  print(config, '\n')
  print(f"logdir: {path.Path(config.logdir)}")
  return config


def load_config_from_raw_yaml(config: pathlib.Path | path.Path | dict) -> Config:
  """Accept a YAML file config with file structures:
    ```yaml
    logroot: path/to/your/logroot
    expname: name_of_your_experiment
