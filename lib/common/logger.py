...
#     TerminalOutput(config.filter),
#     JSONLOutput(logdir, 'metrics.jsonl'),
#     TensorBoardOutput(logdir),
#     # mhs.core.logger.WandBOutput(logdir.name, config=config),
#   ])


def build_logger(config, prefix=None):
  step = Counter()
  logdir = pathlib.Path(config.logdir)
  # multiplier = config.env.get(config.task.split('_')[0], {}).get('repeat', 1)
  outputs = []
  outputs.append(TerminalOutput(config.logger.filter, 'Agent'))
  for output in config.logger.outputs:
    if output == 'jsonl':
      outputs.append(JSONLOutput(logdir, f'{prefix}_metrics.jsonl' if prefix else 'metrics.jsonl'))
      outputs.append(JSONLOutput(logdir, f'{prefix}_scores.jsonl' if prefix else 'scores.jsonl', 'episode/.*'))
      outputs.append(JSONLOutput(logdir, f'{prefix}_eval_scores.jsonl' if prefix else 'eval_scores.jsonl', 'eval_episode/.*'))
    elif output == 'tensorboard':
      outputs.append(TensorBoardOutput(logdir, config.logger.fps))
    elif output == 'expa':
      exp = logdir.split('/')[-4]
      run = '/'.join(logdir.split('/')[-3:])
      proj = 'embodied' if logdir.startswith(('/cns/', 'gs://')) else 'debug'
      outputs.append(ExpaOutput(
          exp, run, proj, config.logger.user, config.flat))
    elif output == 'wandb':
      name = '/'.join(logdir.split('/')[-4:])
      outputs.append(WandBOutput(name))
    # elif output == 'scope':
    #   outputs.append(ScopeOutput(pathlib.Path(logdir)))
    else:
      raise NotImplementedError(output)
  logger = Logger(step, outputs, multiplier=1)#multiplier)
  return logger
