...
#   def __init__(self, data_space, config):
#     self.data_space = data_space
#     self.model = nn.Linear(data_space['y'].shape[0], name="model")
#     self.opt = nn.Optimizer([self.model], lr=1e-3, name="opt")
...
#   def _loss(self, data):
#     x = nn.cast(data['x'])
#     return ((self.model(x).astype(jnp.float32) - data['y'].astype(jnp.float32))**2).mean()

#   def train(self, carry, data):
#     opt_mets = self.opt(self._loss, data, has_aux=False)
#     return carry, {}, opt_mets
