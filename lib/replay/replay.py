...
def build_replay(config, folder_name, mode='train', force_offline=False):
  ...
  directory = pathlib.Path(config.logdir) / folder_name
  if config.replicas > 1:
    directory /= f'{config.replica:05}'
  kwargs = dict(
      length=length, capacity=int(capacity), online=config.replay.online and not force_offline,
      chunksize=config.replay.chunksize, directory=directory, save_wait=True)
...
def build_stream(config, replay, mode):
...
class Replay:
  ...
  def _setseq(self, chunkid, index, values):
    ...
    available = chunk.length - index
    if available >= length:
      with timer.section('set_slice'):
        return chunk.update(index, length, values)
    else:
      with timer.section('set_compose'):
  ...
  def _numitems(self, chunks):
    ...
    if not chunks:
      return 0
    chunks = list(reversed(sorted([pathlib.Path(x).stem for x in chunks])))
