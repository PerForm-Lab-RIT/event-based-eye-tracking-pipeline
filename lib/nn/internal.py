...
def setup(
  ...
  if transfer_guard and jit and not debug_nans:
    global TRANSFER_GUARD_STR
    TRANSFER_GUARD_STR = 'disallow'
    jax.config.update('jax_transfer_guard', TRANSFER_GUARD_STR)
  ...
  utils.COMPUTE_DTYPE = compute_dtype
  utils.PARAM_DTYPE = param_dtype
  global IS_SETUP
  IS_SETUP = True


@contextmanager
def allow_jax_transfer():
...
def grouped_ckpt_fns(params, chunksize):
  ...
  assert sum(len(keys) for keys in groups) == len(params), (sum(len(keys) for keys in groups), len(params))
  assert all(len(keys) for keys in groups), (groups)
