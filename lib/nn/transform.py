...
def pure(fn, nested=False):
  purified = nn.pure(fn, nested=nested)
  def wrapper(params, seed, *args, **kwargs):
    return purified(params, *args, **kwargs, seed=seed)
  return wrapper
...
def init(
  ...
  def init(fun, **jit_kwargs):
    ...
    def wrapper(*args, **kwargs):
      # NOTE: Viet: there is no reason to ignore the state modification here
      #  so I changed it to ignore=False
      # state, out = fun(*args, create=True, modify=True, ignore=False, **kwargs)
